		...
		case FSSH_B_GET_GEOMETRY:
		{
			...
			#elif HAIKU_HOST_PLATFORM_FREEBSD
			{
				...
				if (fstat(fd, &status) == 0) {
					...
					bytesPerCylinder = disklabel.d_nsectors * disklabel.d_secsize * disklabel.d_ntracks;
					disklabel.d_secperunit = mediaSize / disklabel.d_secsize;
					disklabel.d_ncylinders = (mediaSize + bytesPerCylinder - 1) / bytesPerCylinder;
						//	mediaSize / disklabel.d_secsize
